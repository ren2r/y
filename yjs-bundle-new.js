var YBundle = (() => {
  ...
  var assign, keys, forEach, map2, length2, size, isEmpty, every, hasProperty, equalFlat, freeze, deepFreeze;
  ...
  var reconnectTimeoutBase, maxReconnectTimeout, messageReconnectTimeout, setupWS, WebsocketClient;
  ...
  var init_sync = __esm({
    "node_modules/y-protocols/sync.js"() {
      ...
      readSyncMessage = (decoder, encoder, doc2, transactionOrigin) => {
  ...
  var init_y_webrtc = __esm({
    "node_modules/y-webrtc/src/y-webrtc.js"() {
      ...
      readMessage = (room, buf, syncedCallback) => {
  ...
  var init_indexeddb = __esm({
    "node_modules/lib0/indexeddb.js"() {
      ...
      deleteDB = (name) => rtop(indexedDB.deleteDatabase(name));
      createStores = (db, definitions) => definitions.forEach(
        (d) => (
          // @ts-ignore
      ...
      count = (store, range) => rtop(store.count(range));
      get = (store, key) => rtop(store.get(key));
      del = (store, key) => rtop(store.delete(key));
      put = (store, item, key) => rtop(store.put(item, key));
      addAutoKey = (store, item) => rtop(store.add(item));
      getAll = (store, range, limit) => rtop(store.getAll(range, limit));
      queryFirst = (store, query, direction) => {
        let first = null;
      ...
      iterateOnRequest = (request, f) => create4((resolve2, reject2) => {
      ...
      iterateKeys = (store, keyrange, f, direction = "next") => iterateOnRequest(store.openKeyCursor(keyrange, direction), (cursor) => f(cursor.key));
      getStore = (t, store) => t.objectStore(store);
      createIDBKeyRangeUpperBound = (upper, upperOpen) => IDBKeyRange.upperBound(upper, upperOpen);
      createIDBKeyRangeLowerBound = (lower, lowerOpen) => IDBKeyRange.lowerBound(lower, lowerOpen);
    }
  });

  // node_modules/y-indexeddb/src/y-indexeddb.js
  var customStoreName, updatesStoreName, PREFERRED_TRIM_SIZE, fetchUpdates, storeState, IndexeddbPersistence;
  var init_y_indexeddb = __esm({
    "node_modules/y-indexeddb/src/y-indexeddb.js"() {
      ...
      init_promise();
      init_observable();
      customStoreName = "custom";
      updatesStoreName = "updates";
      PREFERRED_TRIM_SIZE = 500;
      fetchUpdates = (idbPersistence, beforeApplyUpdatesCallback = () => {
      ...
      storeState = (idbPersistence, forceStore = true) => fetchUpdates(idbPersistence).then((updatesStore) => {
      ...
      IndexeddbPersistence = class extends Observable {
        /**
         * @param {string} name
         * @param {Y.Doc} doc
         */
  ...
  var messageSync2, messageQueryAwareness2, messageAwareness2, messageAuth, messageHandlers, messageReconnectTimeout2, permissionDeniedHandler, readMessage2, closeWebsocketConnection, setupWS2, broadcastMessage, WebsocketProvider;
...
${reason}`);
      readMessage2 = (provider, buf, emitSynced) => {
        const decoder = createDecoder(buf);
        const encoder = createEncoder();
        const messageType = readVarUint(decoder);
  ...
  var require_bundle = __commonJS({
    "bundle.js"() {
      ...
      init_y_indexeddb();
      init_y_websocket();
      Object.assign(yjs_exports, { WebrtcProvider, IndexeddbPersistence, WebsocketProvider });
      if (typeof window !== "undefined") {
